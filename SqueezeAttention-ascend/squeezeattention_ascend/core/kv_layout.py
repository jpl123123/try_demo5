"""Ascend block-cache KV layout primitives for SqueezeAttention-ascend.

This module converts the kvpress HF-level cache interaction (dense per-layer
``[bsz, H, T, D]`` DynamicCache tensors) into vLLM-Ascend block-cache
operations. Two supported Ascend layouts:

- split cache: ``(k_cache, v_cache)`` with each tensor ``[num_blocks,
  block_size, num_kv_heads, head_dim]``
- combined cache: ``[2, num_blocks, block_size, num_kv_heads, head_dim]``
  (``kv_axis=0``) or ``[num_blocks, 2, block_size, num_kv_heads, head_dim]``
  (``kv_axis=1``)

Adapted from the tri_3_5 reference integration (kv_compaction.py).
"""
...
def _debug_validate_compaction_content() -> bool:
    return os.environ.get("SQUEEZE_DEBUG_VALIDATE_COMPACTION_CONTENT", "0") == "1"
...
def register_kv_layout_axis_hint(kv_cache: torch.Tensor, kv_axis: int) -> None:
    """Register explicit KV axis hint for ambiguous layouts (kv_axis 0 or 1)."""
    if kv_cache.ndim != 5:
...
def split_kv_axes(kv_cache: Any) -> tuple[torch.Tensor, torch.Tensor]:
    """Return key/value views in shape [num_blocks, block_size, H, D]."""
    if isinstance(kv_cache, (tuple, list)):
        if len(kv_cache) < 2:
            raise ValueError(
                "Unsupported split KV layout: expected at least key/value tensors"
            )
        key_cache, value_cache = kv_cache[0], kv_cache[1]
        if not torch.is_tensor(key_cache) or not torch.is_tensor(value_cache):
            raise ValueError("Unsupported split KV layout: entries must be tensors")
        if key_cache.ndim != 4 or value_cache.ndim != 4:
            raise ValueError(
                "Unsupported split KV layout: expected [num_blocks, block_size, H, D]"
            )
    ...
    raise ValueError("Unsupported KV layout for compaction: expected a dimension with size 2")
...
def _consecutive_block_span(
    ...
    if isinstance(block_ids, torch.Tensor):
        ...
        cache_key = (
            ...
            int(block_ids_t.device.index) if block_ids_t.device.type != "cpu" else -1,
        )
...
def _try_dense_token_view(
    ...
    """Return a token-major view [T, H, D] without a gather copy when blocks are
...
def gather_request_kv_dense(
    ...
    This is the Ascend equivalent of kvpress's
    ``extract_keys_and_values(cache, layer_idx)`` (HF DynamicCache dense read).
    """
    if total_tokens <= 0:
        raise ValueError(f"total_tokens must be > 0, got {total_tokens}")

    key_cache, value_cache = split_kv_axes(kv_cache)
    ...
    if key_fast is not None and value_fast is not None:
        return (
            key_fast.transpose(0, 1).unsqueeze(0),
            value_fast.transpose(0, 1).unsqueeze(0),
        )
    ...
    return (
        keys_thd.transpose(0, 1).unsqueeze(0),
        values_thd.transpose(0, 1).unsqueeze(0),
    )
...
def gather_request_k_dense(
    ...
    """Gather one request K from paged cache into dense [1, H, T, D]."""
    if total_tokens <= 0:
        raise ValueError(f"total_tokens must be > 0, got {total_tokens}")

    key_cache, _ = split_kv_axes(kv_cache)
    key_fast = _try_dense_token_view(
        cache_thd=key_cache,
        block_ids=block_ids,
        total_tokens=total_tokens,
...
def build_keep_token_indices(
    ...
    """Build ordered keep indices for one request (recency plan).

    Returns None when the request is impossible under the constraints
    (e.g. protected prefill exceeds the budget).
    """
...
def compact_request_kv_in_place_per_head(
    ...
    This is the Ascend equivalent of kvpress's ``ScorerPress.compress`` gather:
    kept entries are permuted into the first ``keep_count`` token slots of the
    request's existing blocks (per-head sets, uniform count). ``prefix_only``
    writes only the ordered kept prefix (used when the dropped tail blocks are
    physically reclaimed in the same step).
    """
    key_cache, value_cache = split_kv_axes(kv_cache)
    device = key_cache.device
    ...
    if keep_tensor.ndim != 2:
        raise ValueError(f"keep_token_indices_per_head must be 2D, got shape={tuple(keep_tensor.shape)}")
    ...
    if prefix_only:
        ...
        if _debug_validate_compaction_content() and keep_count > 0:
            ...
            if not torch.equal(actual_keys, expected_keys):
                raise RuntimeError("SQUEEZE_DEBUG_COMPACTION_KEY_MISMATCH:per_head_prefix_content_mismatch")
            if not torch.equal(actual_values, expected_values):
                raise RuntimeError("SQUEEZE_DEBUG_COMPACTION_VALUE_MISMATCH:per_head_prefix_content_mismatch")
        return keep_count
    ...
    # fill-hole placement (no dropped-token preservation): only move tail
    # survivors into free prefix slots.
    ...
    if bool(in_prefix.any().item()):
        head_ids_2d = torch.arange(num_kv_heads, device=device, dtype=torch.long).unsqueeze(1)
        head_ids_2d = head_ids_2d.expand(num_kv_heads, keep_count)
        free_mask[head_ids_2d[in_prefix], keep_tensor[in_prefix]] = False
    src_counts = src_mask.sum(dim=1)
    dst_counts = free_mask.sum(dim=1)
    if not torch.equal(src_counts, dst_counts):
        raise RuntimeError("fill_in_place_per_head_mismatch: src/dst counts differ per head")
    ...
    src_tokens = keep_tensor[src_mask]
    dst_tokens = prefix_positions_2d[free_mask]
    head_idx = head_idx_2d[src_mask]
