...
def attach_events_to_output(
    ...
    if output is None:
        if scheduler_output is not None and pending_events:
            setattr(scheduler_output, "squeeze_compression_events", pending_events)
            return output, pending_events
        return output, pending_events
    attach_events_via_kv_connector_output(output, pending_events)
    try:
        setattr(output, "squeeze_compression_events", pending_events)
    except Exception:
        return output, pending_events
    return output, []
...
def read_events_from_output(model_runner_output: Any) -> list[dict[str, Any]] | None:
    ...
    if events is not None:
        return events
    raw = getattr(model_runner_output, "squeeze_compression_events", None)
