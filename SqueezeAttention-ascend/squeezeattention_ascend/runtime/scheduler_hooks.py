"""Scheduler-side hooks for SqueezeAttention-ascend.

SqueezeAttention's HF flow compresses at boundaries tracked by ``cache_position`` and
...
def update_request_effective_kv_offset(request: Any, cache_len_after: int) -> None:
    try:
        setattr(request, "_squeeze_effective_kv_offset", int(cache_len_after))
    except Exception:
        pass
...
def sync_effective_kv_offsets_before_schedule(scheduler: Any, tracker: EffectiveLenTracker) -> None:
    ...
    for req_id, request in requests.items():
        ...
        if not isinstance(num_computed, int) or num_computed <= int(effective):
            continue
        try:
            setattr(request, "_squeeze_orig_num_computed", num_computed)
...
def build_signals(
    ...
    *,
    ...
    SqueezeAttention's ``is_prefilling`` phase gate becomes ``defer_prefill_compression``:
    ...
    if not isinstance(requests, dict):
        return signals
    prefill_lens = getattr(scheduler, "squeeze_prefill_lens", None)
...
def _read_compression_events(
    ...
    if events is not None:
        return events
    raw = getattr(scheduler_output, "squeeze_compression_events", None)
