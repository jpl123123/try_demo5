"""Install SqueezeAttention-ascend monkeypatches into vLLM / vLLM-Ascend.
...
- ``vllm_ascend.worker.worker.NPUWorker``: ``init_device`` / ``execute_model``
  to install the SqueezeAttentionModelRunner proxy;
...
def _patched_scheduler_init(self, *args, **kwargs):
    ...
    cfg = SqueezeRuntimeConfig.from_env()
    self.squeeze_config = cfg
    self._squeeze_effective_len_tracker = EffectiveLenTracker()
    self.squeeze_prefill_lens = {}
    self._squeeze_step = 0
    if cfg.logging_enabled:
        log_info(
            "Scheduler initialized: mode=%s ini_size=%.3f class3=%.3f "
            "start_size=%d min_reclaim_blocks=%d defer_prefill=%s build=%s",
            cfg.mode,
            cfg.ini_size,
            cfg.class3_size,
            cfg.start_size,
...
def _patched_scheduler_schedule(self):
    ...
    # worker-side after prefill (KMeans); the scheduler uses a reclaim-driven
    # candidate (reclaim happens once T*(1 - keep_fraction) exceeds
    # min_reclaim_blocks blocks; keep_fraction ~ ini_size), and the worker
    # validates against the real per-request budgets before compacting.
    if cfg.kv_budget > 0:
        candidate_budget = cfg.kv_budget
    else:
        keep_fraction = max(0.05, min(0.95, float(cfg.ini_size)))
        candidate_budget = max(
            1,
            int(
                max(0, cfg.min_reclaim_blocks)
                * max(1, block_size)
    ...
    signals = build_signals(
        ...
        step=self._squeeze_step,
        log_decisions=cfg.log_decisions,
    )
    setattr(scheduler_output, "squeeze_signals", signals)
    setattr(scheduler_output, "squeeze_step", self._squeeze_step)
    # Scheduler heartbeat: proves the engine-core scheduler patch is alive
    # (TriAttention-style signal accounting), every 500 steps.
    if self._squeeze_step % 500 == 0:
        tracker = getattr(self, "_squeeze_effective_len_tracker", None)
        log_info(
...
def _patched_kv_cache_allocate_slots(self, request, num_new_tokens, *args, **kwargs):
    ...
    effective = getattr(request, "_squeeze_effective_kv_offset", None)
...
def _relax_kv_cache_memory_check() -> None:
    try:
        ...
        if callable(legacy_check) and not getattr(legacy_check, "_squeeze_relaxed", False):
            ...
            def _relaxed_legacy(available_memory, get_needed_memory, max_model_len, estimate_max_model_len):
                ...
                if needed > available_memory:
                    log_info(
                        ...
                        "%.2f GiB available; SqueezeAttention compression keeps actual usage lower.",
                        max_model_len, needed / (1 << 30), available_memory / (1 << 30),
                    )

            setattr(_relaxed_legacy, "_squeeze_relaxed", True)
...
def _install_input_patch() -> bool:
    ...
    try:
        ...
        if callable(original) and not getattr(original, "_squeeze_patched", False):
        ...
        log_warning("could not install SqueezeAttention input patch (v1)", exc_info=True)
    if patched:
        log_info("Installed SqueezeAttention input patches: %s", ", ".join(patched))
    return bool(patched)


def install_squeezeattention_integration_monkeypatches(
    *,
    ...
    if cfg.logging_enabled:
        log_info(
            "Installed SqueezeAttention monkeypatch integration: scheduler=%s worker=%s "
            "mode=%s build=%s",
            patch_scheduler,
            patch_worker,
            cfg.mode,
