"""Ascend block-cache KV layout primitives for kvpress-ascend.
...
def _debug_validate_compaction_content() -> bool:
    return os.environ.get("KVPRESS_DEBUG_VALIDATE_COMPACTION_CONTENT", "0") == "1"
    ...
    if prefix_only:
        ...
        if _debug_validate_compaction_content() and keep_count > 0:
            ...
            if not torch.equal(actual_keys, expected_keys):
                raise RuntimeError("KVPRESS_DEBUG_COMPACTION_KEY_MISMATCH:per_head_prefix_content_mismatch")
            if not torch.equal(actual_values, expected_values):
                raise RuntimeError("KVPRESS_DEBUG_COMPACTION_VALUE_MISMATCH:per_head_prefix_content_mismatch")
        return keep_count
