"""Worker/scheduler block-table synchronization for kvpress-ascend.

After the kvpress press has selected keep indices and the KV cache has been
...
def _debug_disabled() -> bool:
    return os.environ.get("KVPRESS_DEBUG_DISABLE_WORKER_RECLAIM_SYNC", "0").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
