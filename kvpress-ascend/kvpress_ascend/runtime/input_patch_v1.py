"""v1 model-runner input patch for kvpress-ascend.
...
def _runner_positions_np(runner: Any, total: int) -> np.ndarray:
    ...
    raise RuntimeError(
        "KVPRESS_V1_POSITIONS_BUFFER_UNSUPPORTED: cannot read positions numpy view"
    )
...
def _runner_seq_lens_np(runner: Any) -> np.ndarray:
    ...
    raise RuntimeError(
        "KVPRESS_V1_SEQ_LENS_BUFFER_UNSUPPORTED: cannot read seq_lens numpy view"
    )
...
def make_patched_v1_prepare_inputs(
    ...
    setattr(_patched_prepare_inputs, "_kvpress_patched", True)
