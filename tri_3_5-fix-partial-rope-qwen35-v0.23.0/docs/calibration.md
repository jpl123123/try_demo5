...
### Partial-RoPE models (e.g. Qwen3.5)

For models with `partial_rotary_factor < 1` (such as Qwen3.5-27B, where only the
leading `head_dim * partial_rotary_factor` channels per head are rotated), the
stats file must record the partial-RoPE geometry so the runtime only scores the
rotated channels:

- `metadata.head_dim`: full head dimension (e.g. 256).
- `metadata.partial_rotary_factor`: e.g. 0.25 (defaults to 1.0 = full RoPE).
- `metadata.rotary_dim`: number of rotated channels (e.g. 64). If omitted,
  derived as `head_dim * partial_rotary_factor`.
- `metadata.freq_count`: `rotary_dim // 2` (e.g. 32). If omitted, derived as
  `rotary_dim // 2`.
- `metadata.inv_freq`: length-`freq_count` RoPE inverse frequencies.
- Per-head `q_mean_real` / `q_mean_imag` / `q_abs_mean`: length-`freq_count`.

The runtime does **not** infer `rotary_dim` from `inv_freq` length alone —
`partial_rotary_factor` / `rotary_dim` / `freq_count` must be present in the
stats metadata for partial-RoPE models. Calibration must capture query
statistics only over the rotated channels; the standard `scripts/calibrate.py`
does not yet do this automatically for partial-RoPE models, so use a
partial-RoPE-aware calibration flow for such models.
