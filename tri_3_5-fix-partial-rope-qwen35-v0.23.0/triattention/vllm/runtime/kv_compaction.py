...
def _split_kv_axes(kv_cache: Any) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if isinstance(kv_cache, (tuple, list)):
        ...
        if not torch.is_tensor(key_cache) or not torch.is_tensor(value_cache):
            raise ValueError(
                "Unsupported split KV layout: key/value entries must be tensors"
            )
        if key_cache.ndim != 4 or value_cache.ndim != 4:
            raise ValueError(
                "Unsupported split KV layout: expected key/value tensors with "
                f"shape [num_blocks, block_size, H, D], got "
